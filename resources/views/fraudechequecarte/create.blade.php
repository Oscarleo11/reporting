...
<div class="section">
    <div class="section-header">
        ...
        <div class="section-header-breadcrumb">
            <a href="{{ route('fraudechequecarte.index') }}" class="btn btn-outline-primary btn-sm">
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('fraudechequecarte.store') }}">
                            ...
                            <h5 class="mb-3 font-weight-bold">Détails des fraudes</h5>
                                ...
                                <div class="card mb-3 detail-row border border-danger">
                                    <div class="card-body">
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-4">
                                                <label>Code chèque/carte</label>
                                                <select name="details[0][codecheque]" class="form-control" required>
                                                    <option value="">-- Sélectionner --</option>
                                                    @foreach ($familles as $typef)
                                                        <option value="{{ $typef->code }}">{{ $typef->code }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="form-group col-md-4">
                                                <label>Date de fraude</label>
                                                <input type="date" name="details[0][datefraude]" class="form-control" required>
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label>Libellé de la fraude</label>
                                                <input type="text" name="details[0][libellefraude]" class="form-control" required>
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label>Mesures correctives</label>
                                                <input type="text" name="details[0][mesurescorrectives]" class="form-control" required>
                                            </div>
                                            <div class="form-group col-md-6">
                                                <label>Mode opératoire</label>
                                                <input type="text" name="details[0][modeoperatoire]" class="form-control" required>
                                            </div>
                                            <div class="form-group col-md-3">
                                                <label>Nombre de fraudes</label>
                                                <input type="number" name="details[0][nbfraude]" class="form-control" required>
                                            </div>
                                            <div class="form-group col-md-3">
                                                <label>Valeur (CFA)</label>
                                                <input type="number" name="details[0][valeurcfa]" class="form-control" required>
                                            </div>
                                        </div>
