                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-4">
                                        ...
                                        <select name="type" class="form-control" required>
                                            ...
                                            <option value="acquisition" @selected(old('type', $type ?? '') === 'acquisition')>Acquisition de cartes
                                            </option>
                                            <option value="emission" @selected(old('type', $type ?? '') === 'emission')>Émission de cartes
                                            </option>
                                            <option value="fraudechequecarte" @selected(old('type', $type ?? '') === 'fraudechequecarte')>Fraude
                                                chèque/carte</option>
                                            <option value="incidentchequecarte" @selected(old('type', $type ?? '') === 'incidentchequecarte')>Incidents
                                                chèque/carte</option>
                                            <option value="incidentpaiementcarte" @selected(old('type', $type ?? '') === 'incidentpaiementcarte')>Incidents
                                                paiement carte</option>
                                            <option value="incidentpaiementcheque" @selected(old('type', $type ?? '') === 'incidentpaiementcheque')>Incidents
                                                paiement chèque</option>
                                            <option value="reclamationchequecarte" @selected(old('type', $type ?? '') === 'reclamationchequecarte')>Réclamations
                                                chèque/carte</option>
                                            <option value="tarificationchequecarte" @selected(old('type', $type ?? '') === 'tarificationchequecarte')>
                                                Tarification carte / chèque</option>
                                            <option value="typologiecheque" @selected(old('type', $type ?? '') === 'typologiecheque')>Typologie chèques
                                            </option>
                                            <option value="utilisationcarte" @selected(old('type', $type ?? '') === 'utilisationcarte')>Utilisation carte
                                            </option>
                                            <option value="utilisationcheque" @selected(old('type', $type ?? '') === 'utilisationcheque')>Utilisation
                                                chèque</option>
                                            <option value="risquestra" @selected(old('type', $type ?? '') === 'risquestra')>Risques STRA</option>
                                            <option value="incidentstra" @selected(old('type', $type ?? '') === 'incidentstra')>Incidents STRA
                                            </option>
                                            <option value="ecosysteme" @selected(old('type', $type ?? '') === 'ecosysteme')>Écosystème STRA
                                            </option>
                                            <option value="fraudestra" @selected(old('type', $type ?? '') === 'fraudestra')>Fraudes STRA</option>
                                            <option value="operationstra" @selected(old('type', $type ?? '') === 'operationstra')>Opérations STRA
                                            </option>
                                            <option value="reclamationstra" @selected(old('type', $type ?? '') === 'reclamationstra')>Réclamations STRA
                                            </option>
                    ...
                    @isset($donnees)
                        <div class="card shadow-sm border-0">
                            ...
                            <div class="card-body">
                                ...
                                @else
                                    <div class="mb-4">
                                        <h6 class="font-weight-bold">Aperçu des données ({{ $donnees->count() }}
                                            enregistrements)</h6>
